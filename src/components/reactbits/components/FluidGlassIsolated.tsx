...
import { useRef, useState, useEffect, memo, ReactNode, useLayoutEffect } from "react";
import {
  Canvas,
  createPortal,
  useFrame,
...
// Extend THREE to avoid conflicts
extend(THREE);
...
export default function FluidGlass({
  ...
  const [containerId] = useState(() => `fluid-glass-${++instanceCounter}-${Date.now()}`);
  const [canRender, setCanRender] = useState(false);

  useLayoutEffect(() => {
    // Ensure we have a unique container before rendering
    if (containerRef.current) {
      // Clean any existing canvas in this container
